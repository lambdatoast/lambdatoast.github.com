...
The main benefit of FP is **equational reasoning**, which is a direct result of using *functions*. So `square(4)` doesn't "return" `16`. It *is* `16`. It is just another way of *expressing* it. It is an *expression*. An *expression* of `16`.
...
There's more to it, of course. Namely, there are *equational properties* that must be satisfied in order for something to be considered a category. Whatever satisfies those properties can *reliably* use all the categorical tools (machine-wise and brain-wise), proofs, etc. that apply to it.
